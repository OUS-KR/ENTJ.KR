...
const patrolOutcomes = [
    { weight: 30, condition: () => true, effect: (gs) => { const v = getRandomValue(5, 2); return { changes: { order: gs.order + v }, message: `영토 순찰을 통해 질서가 확립되었습니다. (+${v} 질서)` }; } },
...
const gameActions = {
    ...
    meet_subordinates: () => {
        ...
        const possibleOutcomes = meetOutcomes.filter(o => !o.condition || o.condition(gameState, subordinate));
        const totalWeight = possibleOutcomes.reduce((sum, o) => sum + o.weight, 0);
        const rand = currentRandFn() * totalWeight;
        let cumulativeWeight = 0;
        let chosenOutcome = possibleOutcomes.find(o => (cumulativeWeight += o.weight) >= rand) || possibleOutcomes[0];
        const result = chosenOutcome.effect(gameState, subordinate);
        updateState(result.changes, result.message);
    },
    issue_decree: () => {
        if (!spendActionPoint()) return;
        const possibleOutcomes = decreeOutcomes.filter(o => !o.condition || o.condition(gameState));
        const totalWeight = possibleOutcomes.reduce((sum, o) => sum + o.weight, 0);
        const rand = currentRandFn() * totalWeight;
        let cumulativeWeight = 0;
        let chosenOutcome = possibleOutcomes.find(o => (cumulativeWeight += o.weight) >= rand) || possibleOutcomes[0];
